...
/**
 * If this is true, when any other thread than the message thread tries to access a function, then this will assert.
 *
 * Default: 1
 */
...
{
    /* You should really be only calling this on the message thread.
       If you desire to add a processor on a thread which is not the main event dispatch loop,
 ...
 *  Beware to only use this when you really know what you're doing, as calling message thread code on the audio thread
...
/**
 *  Sets the current mouse cursor and resets it to the passed cursor or the normal cursor if omitted.
 */
...
struct ScopedCursorWait : ScopedCursor { ScopedCursorWait() : ScopedCursor(MouseCursor::WaitCursor) {} };
